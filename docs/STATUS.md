...
P3 迭代 1「预算体系 + 超支提示」已完成。下一步:P3 迭代 2「周期支出识别 + 到期提醒」(见路线图)。
...
- **P3 剩余**:周期支出识别 + 到期提醒(迭代 2)、净资产总览 + 月度报告(迭代 3)、异常检测 + 数据备份(迭代 4)
- 部署方案未定(当前仅本地开发环境)
...
- 测试:35 E2E(TC-N 2 / TC-T 7 / TC-O 5 / TC-B 5 / TC-F 7 / TC-A 4 / TC-BG 5)+ 7 单测,最近一次全绿
- 数据表:8(新增 budgets);E2E 用例组:7
