...
- **单机版(无后端)**:交互演示版升级为功能对齐、可日常使用的单机版(自包含单 HTML + localStorage,双击即开、离线可用)——补齐净资产总览(现金+基金构成条)、月度报告(结余/环比/Top/最大单笔/预算执行,月份切换)、异常支出检测(同 ADR 0014 规则,首页提醒条+分析页区块)、分类管理(新增/改名同步历史/删除受使用中保护)、导出 CSV(UTF-8 BOM+防公式注入,Blob 下载)、AI 问答(本地规则即时计算,不调大模型);「我的」重组为 分析与预算/数据管理 两组;旧 localStorage 数据自动迁移;冒烟链扩至全功能并浅/深主题目检。真实 OCR/AI 需服务端持有 Key,属无后端形态的固有边界,已在 docs/演示版.md 诚实说明。
